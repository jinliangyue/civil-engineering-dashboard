...
> 基于国家统计局公开的工业生产者出厂价格指数（PPI），构建了 4 大工业行业（黑色金属冶炼、有色金属冶炼、黑色金属矿采选、有色金属矿采选）的跨年度价格走势分析平台 + 月度时间序列预测模块。覆盖 11 年 × 4 行业 = 44 个年度数据点 + akshare 抓取的 132 个月度真实数据点，实现了长期趋势分析、跨行业相关性矩阵、同比变动、2026-2028 线性回归预测、机器学习多模型对比（Prophet + XGBoost + LSTM 三模型，特征工程含滞后/滚动统计/同比环比）6 大功能。基于 Python + pandas + Plotly + Streamlit + TensorFlow + Prophet 全栈实现，已部署到 Streamlit Cloud 提供在线交互式仪表盘。

## 简历项目栏（100 字精简版）

> 基于国家统计局公开 PPI 数据，构建了 4 大工业行业跨年度 + 月度时间序列分析平台。实现了趋势分析、跨行业相关性、月度 3 模型预测（Prophet / XGBoost / LSTM）共 6 大功能，覆盖 44 年度 + 132 月度数据点。基于 Python + Plotly + Streamlit + TensorFlow 全栈实现并部署上线。
...
- 月度时间序列预测中 Prophet R²=0.79 表现最佳（捕获年度季节性）+ XGBoost MAPE=0.28% 最精准
